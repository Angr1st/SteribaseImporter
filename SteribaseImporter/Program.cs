...
namespace SteribaseImporter
{
    class Program
    {
        static void Main(string[] args)
        {
            ...
            var importResults = mover.LoadAllNewXmls().Select(doc => (processor.ImportXml(doc.xmlDoc), doc.filePath)).Select(import => { WriteLine($"Import of {mover.GetFileName(import.filePath)}; Successfull:{import.Item1.erfolgreich}; Failed:{import.Item1.fehlerhaft}"); return import; }).Select(import => (mover.MoveFile(import.filePath), import)).Select(import => mover.CreateErrorLog(import.import.Item1)).ToList();
            ReadLine();
        }
    }
}
